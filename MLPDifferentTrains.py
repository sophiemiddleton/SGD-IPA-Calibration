...
y_pred = regr.predict(Xnew)

#Write out the prediction
pd.DataFrame(y_pred).to_csv("predicteddEUnfilteredMLP.csv", index=False)

# print results and make simple plot:
print('reg score: ',regr.score(Xnew,Ynew))
...
print('varience score :',metrics.explained_variance_score(Ynew, y_pred))
print('r2', np.sqrt(metrics.r2_score(Ynew, y_pred)))
#plot:

fig, ax = plt.subplots()
ax.scatter(Ynew, abs(y_pred))
